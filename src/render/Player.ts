...
function drawPathDelta(ctx:CanvasRenderingContext2D,u:DrawUnit,a:number,b:number,tile:HTMLCanvasElement){
  ...
  mask.globalCompositeOperation='source-over';mask.strokeStyle='#fff';mask.lineWidth=3;mask.lineCap='round';mask.lineJoin='round';mask.beginPath();mask.moveTo(u.path[from*2]-u.bbox.x,u.path[from*2+1]-u.bbox.y)
  for(let i=from+1;i<=Math.min(to,n-1);i++)mask.lineTo(u.path[i*2]-u.bbox.x,u.path[i*2+1]-u.bbox.y)
  mask.stroke();mask.globalCompositeOperation='source-in';mask.drawImage(tile,0,0);mask.globalCompositeOperation='source-over'
...
function unitPosition(u:DrawUnit,p:number){const q=unitProgress(u,p),n=u.path.length/2;if(u.type==='path'&&n){const i=Math.min(n-1,Math.floor(q*(n-1)));return{x:u.path[i*2],y:u.path[i*2+1]}}return{x:u.bbox.x+u.bbox.w*q,y:u.bbox.y+u.bbox.h/2}}
...
function drawPushEntry(display:CanvasRenderingContext2D,analysis:AnalysisResult,units:DrawUnit[],rgba:Uint8Array,bg:[number,number,number],w:number,h:number,progress:number,crop:{x:number;y:number;w:number;h:number},push:FrameSettings['objectPushEntry']|undefined,cache:Map<number,HTMLCanvasElement>){
  if(!push?.selectedBlockIds.length)return
  for(const id of push.selectedBlockIds){const block=analysis.blocks.find(b=>b.id===id);if(!block)continue;const own=units.filter(u=>u.blockId===id),start=own[0]?.t0,end=own.at(-1)?.t1;if(start===undefined||end===undefined)continue;const entryEnd=start+(end-start)*.25;if(progress<start||progress>=entryEnd)continue;const q=(progress-start)/Math.max(.0001,entryEnd-start),ease=1-Math.pow(1-q,3),edge=push.edgeOverrides[id]??'auto',tile=cache.get(id)??getBlockTile(block,rgba,bg,w);cache.set(id,tile);let dx=0,dy=0;const chosen=edge==='auto'?nearestEdge(block.bbox,w,h):edge;if(chosen==='left')dx=-block.bbox.w*(1-ease)-block.bbox.x;if(chosen==='right')dx=(w-block.bbox.x)*(1-ease);if(chosen==='top')dy=-block.bbox.h*(1-ease)-block.bbox.y;if(chosen==='bottom')dy=(h-block.bbox.y)*(1-ease);display.save();display.globalAlpha=.65*ease;display.drawImage(tile,(block.bbox.x+dx-crop.x)*w/crop.w,(block.bbox.y+dy-crop.y)*h/crop.h,block.bbox.w*w/crop.w,block.bbox.h*h/crop.h);display.restore()}
}
