...
function normalizeAnalysisResult(result: AnalysisResult): AnalysisResult {
  ...
  const diagnostics = result.diagnostics ? { ...result.diagnostics, finalObjectCount: objects?.length ?? result.diagnostics.finalObjectCount, objectCount: objects?.length ?? result.diagnostics.objectCount, coveragePixelCount: result.stats.coveragePixels ?? result.diagnostics.coveragePixelCount, reconstruction: ownership.exact ? 'exact' as const : ownership.duplicatePixels ? 'overlap' as const : 'incomplete' as const, reconstructionMismatch: ownership.duplicatePixels + ownership.missingPixels, warnings: ownership.exact ? result.diagnostics.warnings : [...result.diagnostics.warnings, `Ownership invariant failed: ${ownership.duplicatePixels} duplicate, ${ownership.missingPixels} missing pixels.`] } : undefined
