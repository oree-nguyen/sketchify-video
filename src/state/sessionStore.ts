...
export async function restoreProject(serialized: SerializedProject): Promise<Project> {
  ...
  try {
    const frames = await Promise.all([...serialized.frames].sort((a, b) => a.order - b.order).map(async (frame) => {
      ...
      const settings = { ...structuredClone(DEFAULT_SETTINGS), ...frame.settings, camera: { ...DEFAULT_SETTINGS.camera, ...frame.settings.camera }, pageZoom: { ...DEFAULT_SETTINGS.pageZoom, ...frame.settings.pageZoom }, cameraPinned: frame.settings.cameraPinned ?? false, rasterWipe: frame.settings.rasterWipe ?? null, rasterWipeDurationSec: frame.settings.rasterWipeDurationSec ?? DEFAULT_SETTINGS.rasterWipeDurationSec }
